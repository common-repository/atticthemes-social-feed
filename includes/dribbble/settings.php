...
<div class="atsf-main atsf-dribbble"><?php
	$user = self::get_user();
	...
	if( !self::get_access_token() || !$user ) {
		?><div class="atsf-screen atsf-auth-screen"><?php
			...
			?><div class="atsf-sign-in-wrap">
				<a class="atsf-sign-in" href="<?php echo esc_url($auth_url); ?>"><?php
					_e('<strong>Connect</strong> to Dribbble', 'atsf');
				?>
		...
		?><div class="atsf-screen atsf-settings-screen">
			...
			<span class="atsf-help"><?php
				echo apply_filters( 'atsf_help_message', __('Now all you need to do is copy the shortcode below and paste it in places where you want to display your Dribbble shots.', 'atsf'), $user );
			?></span><?php
			...
			*/
			...
			$shortcode = '[atsf_dribbble count="' . rand(1, intval($user->counts->media)) . '"]';
			...
			if( $media ) {
				...
				?><div class="atsf-sample-media">
					<div class="atsf-error atsf-popup"><?php
						_e('Oh dear! Something went wrong, couldn\'t get the feed. <br> Try clearing the cache or reconnect with Dribbble.', 'atsf');
					?></div>
				</div><?php
			} // end if media*/


			$clear_message = __('Clear the cache only when you have new posts and the plugin has not yet retrived them from Dribbble. Do not use this too often or Dribbble might block you for making to many requests.', 'atsf');

			$logout_message = __('This is going to disconnect you from Dribbble. You will need to re-authenticate again. If you want to change users, make sure to log out from Dribbble too.', 'atsf');


			?><form class="atsf-cache" method="post" action="<?php echo esc_url($admin_url); ?>"> 
				<button type="submit" class="button" name="clear_cache" value="dribbble"
					onclick='return confirm("<?php echo $clear_message; ?>")'><?php _e('Clear the cache', 'atsf'); ?></button>

				<button type="submit" class="button" name="logout" value="dribbble"
